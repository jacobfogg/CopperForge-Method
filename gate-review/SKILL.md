...
5. **(Code Gate only) Insert §3 Push-pending disclosure and §4 Evidence.** Both sections are Code-Gate-only and load only on Code Gate invocations; see [`references/code-gate.md`](references/code-gate.md) for the literal heading text, the push-pending disclosure rule (binding per the [COPAAA-12](/COPAAA/issues/COPAAA-12) anchor-gate hand-validation precedent), and the verbatim-Artifact-A–D evidence rule. Skip on Plan / Test / Epic Gates.
