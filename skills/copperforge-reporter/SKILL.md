...
- The routine that spawned the execution issue is registered in the reporter agent's company per the Paperclip routines lifecycle (BA [research-summary §3.2](/COPAAA/issues/COPAAA-62#document-research-summary)). v1 registers exactly one routine — the blocker-trigger schedule trigger — per `references/blocker-trigger.md`. Routine registration is a one-time CEO step (`agents:create` permission — skill-library mutations are CEO-only per [COPAAA-58](/COPAAA/issues/COPAAA-58) and BA [research-summary §5.3](/COPAAA/issues/COPAAA-62#document-research-summary)); it is NOT performed inside this skill body.
...
- Per-skill README row update is mandatory at Code Gate per the per-skill README row rule ratified at [COPAAA-19 B4](/COPAAA/issues/COPAAA-19) (mirrored at [COPAAA-83](/COPAAA/issues/COPAAA-83) §4.2 Rule C/D scope). The `copperforge-reporter` row in repo `README.md` lands in the same coding commit as this SKILL.md and is reaffirmed in the Code Gate evidence package.
