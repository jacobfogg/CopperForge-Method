...
2. **Resolve parent set.**
   ...
   - If `parentIds` is `null` or empty, auto-discover by issuing `GET /api/companies/{companyId}/dashboard`. The `/dashboard` endpoint returns the company's top-level structure per BA research-summary §2.5; the response contract used here is the `epics[]` array (top-level issues with `parentIssueId === null` and at least one child). Each entry's `id` is added to the parent set. The auto-discovery path tracks the methodology's evolving epic set without CEO re-registration.
